...
`top` 명령어는 시스템의 전체 프로세스 상태와 리소스(CPU, Memory) 사용량을 **실시간(Real-time)**으로 모니터링할 때 사용합니다. 3초(기본값)마다 화면이 갱신되며, 시스템 부하가 높을 때 원인을 파악하기 가장 좋은 도구입니다.
