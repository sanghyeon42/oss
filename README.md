...
| 단축키 | 기능 | 설명 |
| :---: | :---: | :--- |
| **`q`** | **종료 (Quit)** | `top` 프로그램을 종료합니다. |
| **`k`** | **프로세스 종료** | 종료할 PID를 입력하여 프로세스를 제어합니다. |
| **`1`** | **CPU 코어별 보기** | 논리 코어별 CPU 사용량을 펼치거나 접습니다. |
| **`M`** | **메모리 정렬** | 메모리 사용량(RES) 순으로 내림차순 정렬합니다. |
| **`P`** | **CPU 정렬** | CPU 사용량 순으로 내림차순 정렬합니다. (기본값) |
